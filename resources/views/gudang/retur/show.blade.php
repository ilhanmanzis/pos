<x-layout>
    ...
    <div class="p-8">
        ...
        <!-- Breadcrumb Start -->
        <div x-data="{ pageName: `Edit Retur` }">
        ...
        <div class="mx-5 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="space-y-6 border-t border-gray-100 px-5 sm:px-6 dark:border-gray-800" x-data="returForm()"
                ...
                <form @submit.prevent="submitForm"
                    action="{{ route('gudang.retur.update', ['id' => $retur['id_retur']]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <!-- Pilih Produk -->
                    <div class="my-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative bg-transparent">
                                <select name="kondisi"
                                    ...
                                    <option {{ $retur->jenis === 'bagus' ? 'selected' : '' }} value="bagus"
                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        Bagus
                                    </option>
                                    <option {{ $retur->jenis === 'rusak' ? 'selected' : '' }} value="rusak"
                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        Rusak
                                    </option>
                        ...
                        <div>
                            ...
                            <textarea placeholder="........" type="text" rows="6" name="catatan"
                                class="dark:bg-dark-900 shadow-theme-xs focus:ring-brand-500/10  w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 {{ $errors->has('catatan') ? 'border-error-300 focus:border-error-300 dark:border-error-700 dark:focus:border-error-800' : 'border-gray-300 focus:border-brand-300 dark:border-gray-700' }}">{{ $retur['catatan'] }}</textarea>
                            @error('catatan')
                                <p class="text-theme-xs text-error-500 my-1.5">
                                    {{ $message }}
                                </p>
                ...
                <script>
                    ...
                    function returForm() {
                        return {
                            selectedProduk: '{{ $retur->details[0]->id_produk ?? '' }}',
                            sizes: [],
                            init() {
                                this.sizes = @json($returDetails->toArray());
                            },
                            loadStoks() {
